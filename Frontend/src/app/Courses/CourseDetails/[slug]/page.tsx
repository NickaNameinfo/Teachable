...
const CourseDetails = ({ params }: { params: { slug: string } }) => {
  ...
  const { data: courseWatchlistData, refetch: courseWatchlistRefetch } =
    useGetWatchlistCouresIdQuery(params.slug);
  ...
  const { data: sessionList, refetch: refetchSessionList } =
    useGetSessionsIdQuery(params.slug);
  const { data: feebacklist, refetch: refetchFeedbackList } =
    useGetFeebackCouresIdQuery(params.slug);
  const { data: checkOutItem, refetch: refetchCheckOutItem } =
    useGetCheckOutCouresIdQuery({
      courseId: params.slug,
      customerId: userInfo?.id,
    });
  console.log(feebacklist, "asdfsadfasdfeebacklist", params.slug);
  const {
  ...
  const VideoComponent = ({ result, infoData, index }) => {
    ...
    return (
      <video
        ...
        id={result?.id}
      >
        <source src={`${infoData?.baseApi}/${result?.sessionUrl}`} type="video/mp4" />
